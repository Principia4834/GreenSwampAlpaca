...
- Prefer strictly evidence-based analysis and do not make speculative assertions without direct support from code/logs. Ask for missing facts explicitly before concluding and provide corrective observations (e.g., exact line numbers from downloaded CSS) that should be treated as authoritative during debugging.
...
- Validate assumptions using browser DevTools results (e.g., $0.closest selectors, computed styles) and expect fixes based on those observations.
