...
namespace GreenSwamp.Alpaca.Mount.SkyWatcher
{
    ...
    public class SkyWatcher
    {
        ...
        /// </summary>
        /// <param name="axis">>Axis.Axis1 or Axis.Axis2</param>
        ...
        internal void AxisPulse(Axis axis, double guideRate, int duration, int backlashSteps, CancellationToken token)
        {
            Task.Run(() =>
            {
                var datetime = HiResDateTime.UtcNow;
                var monitorItem = new MonitorEntry // setup to log the pulse
                { Datetime = datetime, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Debug, Method = MonitorLog.GetCurrentMethod(), Thread = Environment.CurrentManagedThreadId, Message = $"{axis}|{guideRate}|{duration}|{backlashSteps}|{MinPulseDurationRa}|{MinPulseDurationDec}|{DecPulseGoTo}" };
                LogMonitor(monitorItem);
                ...
                switch (axis)
                {
                    case Axis.Axis1:
                        ...
                        try
                        {
                            ...
                            if (_pPecOn && AlternatingPPec) { SetPPec(Axis.Axis1, false); } // implements the alternating pPEC 
                            ...
                            if (_commands.SupportAdvancedCommandSet && _commands.AllowAdvancedCommandSet)
                            {
                                _commands.AxisSlew_Advanced(Axis.Axis1, applyRate);
                            }
                            else
                            {
                                var speedInt = CalculateSpeed(Axis.Axis1, applyRate);  // Calculate required mount speed
                                AxisStatus axesStatus = _commands.GetAxisStatus(Axis.Axis1); // Get axis status
                                ...
                                {
                                    _commands.AxisStop(Axis.Axis1); // Stop axis motion
                                    _commands.SetMotionMode(Axis.Axis1, 1, axesStatus.SlewingForward ? 1 : 0, false);
                                    _commands.SetStepSpeed(Axis.Axis1, speedInt); // :I Send pulse to axis
                                    _commands.StartMotion(Axis.Axis1);
                                }
                                else
                                {
                                    _commands.SetStepSpeed(Axis.Axis1, speedInt); // :I Send pulse to axis
                        ...
                        finally
                        {
                            ...
                            if (_commands.SupportAdvancedCommandSet && _commands.AllowAdvancedCommandSet)
                            {
                                _commands.AxisSlew_Advanced(Axis.Axis1, trackingRateRa);    
                            }
                            else
                            {
                                if (changeDirection)
                                {
                                    _commands.AxisStop(Axis.Axis1); // Stop axis motion
                                    _commands.SetMotionMode(Axis.Axis1, 1, slewingForward ? 0 : 1, false);
                                    _commands.SetStepSpeed(Axis.Axis1, trackingSpeedRa); // :I Send pulse to axis
                                    _commands.StartMotion(Axis.Axis1);
                                }
                                else
                                {
                                    _commands.SetStepSpeed(Axis.Axis1, trackingSpeedRa); // :I set speed back to previous tracking speed
                                }
                            }
                    ...
                    case Axis.Axis2:
                        ...
                        else
                        {
                            try
                            {
                                ...
                                // check for cancellation
                                token.ThrowIfCancellationRequested();
                                AxisSlew(Axis.Axis2, guideRate); // Send pulse to axis 
...
//                                      if (sw3.ElapsedMilliseconds % 200 == 0) { UpdateSteps(); } // Process positions while waiting
                ...
                LogPulse(pulseEntry);//send to monitor
            }, token);
        }
        ...
        internal void AxisMoveSteps(Axis axis, long movingSteps)
        {
            var monitorItem = new MonitorEntry
                { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Debug, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Environment.CurrentManagedThreadId, Message = $"{axis}|{movingSteps}" };
            LogMonitor(monitorItem);
        ...
        internal void AxisGoToTarget(Axis axis, double targetPosition) 
        {
            var monitorItem = new MonitorEntry
                { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Debug, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Environment.CurrentManagedThreadId, Message = $"{axis}|{targetPosition}" };
            LogMonitor(monitorItem);
        ...
        internal double[] GetPositionsInDegrees()
        {
            ...
            var x = _commands.GetAxisPositionNaN(Axis.Axis1);
            if (!double.IsNaN(x)) { x = Units.Rad2Deg1(x); }
            positions[0] = x;

            var y = _commands.GetAxisPositionNaN(Axis.Axis2);
        ...
        internal double[] GetSteps()
        {
            ...
            var x = _commands.GetAxisStepsNaN(Axis.Axis1);
            positions[0] = x;

            var y = _commands.GetAxisStepsNaN(Axis.Axis2);
            positions[1] = y;
        ...
        internal void UpdateSteps()
        {
            ...
            {
                var monitorItem = new MonitorEntry
                    { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Information, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Environment.CurrentManagedThreadId, Message = $"{e.Message}" };
                LogMonitor(monitorItem);
            }
        }
        ...
        internal string[] GetAxisStringVersions()
        {
            ...
            var monitorItem = new MonitorEntry
            { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Information, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Environment.CurrentManagedThreadId, Message = $"{ret[0]}|{ret[1]}" };
            LogMonitor(monitorItem);
        ...
        /// <param name="axis">Axis.Axis1 or Axis.Axis2</param>
        /// <returns></returns>
        internal AxisStatus GetAxisStatus(Axis axis)
        {
        ...
        /// <param name="axis">Axis.Axis1 or Axis.Axis2</param>
        /// <returns></returns>
        internal AxisStatus GetCacheAxisStatus(Axis axis)
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal long GetSiderealRate(Axis axis)
        {
        ...
        internal void SetAxisPosition(Axis axis, double newValue)
        {
        ...
        internal void SetAxisPositionCounter(Axis axis, int newValue)
        {
        ...
        internal void SetEncoder(Axis axis, bool on)
        {
        ...
        /// <param name="axis">Axis.Axis1 or Axis.Axis2</param>
        /// <param name="on">on=true,off=false</param>
        internal string? SetPPec(Axis axis, bool on)
        {
        ...
        /// <param name="axis">Axis.Axis1 or Axis.Axis2</param>
        /// <param name="on">on=true,off=false</param>
        internal void SetPPecTrain(Axis axis, bool on)
        {
            if (!CanPPec) { return; }
        ...
        internal double GetAxisPosition(Axis axis)
        {
        ...
        internal long GetAxisPositionCounter(Axis axis, bool raw = false)
        {
        ...
        internal double GetRampDownRange(Axis axis)
        {
            return _commands.GetRampDownRange(axis);
        }

        internal double GetEncoderCount(Axis axis)
        {
            return _commands.GetEncoderCount(axis);
        }

        internal double GetLastGoToTarget(Axis axis)
        {
            return _commands.GetLastGoToTarget(axis);
        }

        internal long GetLastSlewSpeed(Axis axis)
        {
            return _commands.GetLastSlewSpeed(axis);
        }

        internal long? GetHomePosition(Axis axis)
        {
            return !CanHomeSensors ? ((long?)null).GetValueOrDefault() : _commands.GetHomePosition(axis);
        }

        internal string GetMotorCardVersion(Axis axis)
        {
        ...
        internal void SetMotionMode(Axis axis, int func, int direction)
        {
            _commands.SetMotionMode(axis, func, direction, SouthernHemisphere);
        }

        internal void StartMotion(Axis axis)
        {
            _commands.StartMotion(axis);
        }

        internal void SetGotoTargetIncrement(Axis axis, long stepsCount)
        {
            _commands.SetGotoTargetIncrement(axis, stepsCount);
        }

        internal void SetStepSpeed(Axis axis, long stepSpeed)
        {
            _commands.SetStepSpeed(axis, stepSpeed);
        }

        internal void SetBreakPointIncrement(Axis axis, long stepsCount)
        {
        ...
        internal void SetTargetPosition(Axis axis, double position)
        {
            _commands.SetTargetPosition(axis, position);
        }

        internal void SetPolarLedLevel(Axis axis, int level)
        {
        ...
        private void SetRates(Axis axis, double rate)
        {
            if (axis == Axis.Axis1)
            {
                _trackingRates[0] = rate;
                //_trackingSpeeds[0] = CalculateSpeed(Axis.Axis1, rate);
            }
            else
            {
                _trackingRates[1] = rate;
                //_trackingSpeeds[1] = CalculateSpeed(Axis.Axis2, rate);
            }
        ...
        internal void SetHomePositionIndex(Axis axis)
        {
        ...
        internal double Get_j(Axis axis, bool raw = false)
        {
        ...
        internal int GetAngleToStep(Axis axis, double angleInRad)
        {
            return _commands.AngleToStep(axis, angleInRad);
        }

        internal double GetStepToAngle(Axis axis, long steps)
        {
        ...
        /// <returns></returns>
        private long RadSpeedToInt(Axis axis, double rateInRad)
        {
            try
            {
                _factorRadRateToInt = _commands.GetFactorRadRateToInt();
        ...
        {
            string? result = _commands.GetCapabilities(Axis.Axis1);
            Capabilities = result;
            if (result == null || result.Contains("!") || result.Length < 3 || !result.Contains("="))
            {
                var monitorItem = new MonitorEntry
                { Datetime = HiResDateTime.UtcNow, Device = MonitorDevice.Telescope, Category = MonitorCategory.Mount, Type = MonitorType.Warning, Method = MethodBase.GetCurrentMethod()?.Name, Thread = Environment.CurrentManagedThreadId, Message = $"{result}" };
                LogMonitor(monitorItem);
                return;
            }
        ...
        /// <returns></returns>
        private long CalculateSpeed(Axis axis, double rate)
        {
